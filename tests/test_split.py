...
def test_split_only_emits_train_or_test():
    for sid in range(200):
        assert split_for_subject(sid) in {"train", "test"}


def test_split_ratio_is_roughly_80_20_on_large_sample():
    n = 50_000
...
def test_cohort_dataset_split_partition_is_disjoint_and_exhaustive(tmp_path):
    ...
    assert train_starts | test_starts == full_starts
